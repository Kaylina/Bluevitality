#!/bin/bash
#tomcat: 版本：tomcat:8.0.43-jre8、需要先从官网下载tomcat镜像
#脚本执行时：WAR包、镜像需要在脚本同一目录下(流程：设置变量、导入镜像、挂载目录、运行容器)
#挂载的目录默认在：/data/docker-tomcat


HOST_TOM_PORT=8089      #Tomcat与宿主机关联的端口
...
#若已存在此容器先删除（本脚本没有使用dockerfile方式）
