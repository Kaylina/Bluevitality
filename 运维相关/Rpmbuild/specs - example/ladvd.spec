...
%if 0%{?rhel} < 6
BuildRequires: 	gcc44-c++, gcc44, libevent2-devel
%else
...
export CFLAGS="$RPM_OPT_FLAGS -lresolv"
export CXXFLAGS="$RPM_OPT_FLAGS -lresolv"
export AUXLIBS="-lresolv"
%if 0%{?rhel} < 6
%configure CXX=/usr/bin/g++44 CC=/usr/bin/gcc44 --docdir=%{_docdir}/%{pkgname} %{?configure_args}
%else
...
/usr/sbin/useradd  -r -s /sbin/nologin -d %{homedir} -M -c '%{gecos}' -g %{pkgname} %{pkgname} &>/dev/null || :
