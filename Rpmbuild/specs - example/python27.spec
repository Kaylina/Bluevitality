...
%global pylibdir %{_exec_prefix}/lib/python%{pybasever}
...
# Patch setup.py so that it links against the respective version of db4:
...
rm -f python2 %{buildroot}%{_bindir}/python2 %{buildroot}%{_bindir}/python27 %{buildroot}%{_bindir}/python
cp %{buildroot}%{_bindir}/python2.7 %{buildroot}%{_bindir}/python27
%if %{main_python}
ln -s python %{buildroot}%{_bindir}/python2_27
%if 0%{?with_debug_build}
ln -s python-debug %{buildroot}%{_bindir}/python2-debug
%endif # with_debug_build
%else
#mv %{buildroot}%{_bindir}/python %{buildroot}%{_bindir}/%{python}
mv %{buildroot}%{_bindir}/python-config %{buildroot}%{_bindir}/%{python}-config
%if 0%{?with_debug_build}
...
rm -f %{buildroot}/%{_mandir}/man1/python.1
%if 0%{?fedora} >= 14
...
install -p -m 644 %{SOURCE6} %{buildroot}%{_sysconfdir}/rpm/macros.python27
...
CheckPython() {
  ...
  #  ----------------------------------------------------------------------
  #
  # FIXME: test_pty fails in mock on RHEL 6, so excluding for now.
...
%{dynload_dir}/*.so
#%exlude /usr/lib/python2.7/lib-dynload/_testcapimodule.so
...
%{_exec_prefix}/lib/%{py_INSTSONAME_optimized}
%if 0%{?with_systemtap}
...
%files devel
%defattr(-,root,root,-)
%{_exec_prefix}/lib/pkgconfig/python-%{pybasever}.pc
%{_exec_prefix}/lib//pkgconfig/python.pc
%{_exec_prefix}/lib//pkgconfig/python2.pc
%{pylibdir}/config/*
...
%{_exec_prefix}/lib/libpython%{pybasever}.so
...
%{pylibdir}/libpython%{pybasever}_d.so
...
%changelog
* Wed Sep 11 2013 Ivan Polonevich <joni @ wargaming dot net> - 2.7.1-9
- Rebuild fo WG

* Fri Jul 1 2011 Daniel Riek <daniel.riek@vincorex.ch> - 2.7.1-7vx5
- Added an explicit Requires for expat2 as it did not work otherwise
- Moved to expat2 with -devel contents relocated to expat2 subdir to avoid 
  conflict with the system expat package on CentOS 5.5.

* Fri Jun 24 2011 Daniel Riek <daniel.riek@vincorex.ch> - 2.7.1-7vx2
- Imported the bytecode compilation hacks from the EPEL python26 package.
- Modifications to build on RHEL 5 and 6 as secondary python
- Excluding test_pty and test_uuid1 as they fails in mock on RHEL 6
